...
@Service
public class GenerateLoanReceipt {

    private final PdfDocumentDesignRepository pdfDocumentDesignRepository;

    public GenerateLoanReceipt(PdfDocumentDesignRepository pdfDocumentDesignRepository) {
        this.pdfDocumentDesignRepository = pdfDocumentDesignRepository;
    }

    public ByteArrayOutputStream generate(LoanReceipt loanReceipt) throws Exception {
        return generatePDFFromJavaObject(loanReceipt);
    }

    private ByteArrayOutputStream generatePDFFromJavaObject(LoanReceipt loanReceipt) throws Exception {

        ByteArrayOutputStream xmlSource = getXMLSource(loanReceipt);
    ...
    private ByteArrayOutputStream getXMLSource(LoanReceipt loanReceipt) throws Exception {
        JAXBContext context;
        ByteArrayOutputStream outStream = new ByteArrayOutputStream();
        try {
            context = JAXBContext.newInstance(LoanReceipt.class);
            Marshaller marshaller = context.createMarshaller();
            marshaller.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, Boolean.TRUE);
            marshaller.marshal(loanReceipt, outStream);
    ...
    private ByteArrayOutputStream  generatePDF(StreamSource streamSource) {
        ...
        Optional<PdfDocumentDesign> optional = pdfDocumentDesignRepository
                .findByDocumentType(PdfDocumentDesign.LOAN_RECEIPT);
