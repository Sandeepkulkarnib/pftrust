...
@Service
public class GenerateLoanHistorySheet {

    private final PdfDocumentDesignRepository pdfDocumentDesignRepository;

    public GenerateLoanHistorySheet(PdfDocumentDesignRepository pdfDocumentDesignRepository) {
        this.pdfDocumentDesignRepository = pdfDocumentDesignRepository;
    }

    public ByteArrayOutputStream generate(LoanHistorySheet loanHistorySheet) throws Exception {
        return generatePDFFromJavaObject(loanHistorySheet);
    }

    private ByteArrayOutputStream generatePDFFromJavaObject(LoanHistorySheet loanHistorySheet) throws Exception {

        ByteArrayOutputStream xmlSource = getXMLSource(loanHistorySheet);
    ...
    private ByteArrayOutputStream getXMLSource(LoanHistorySheet loanHistorySheet) {
        JAXBContext context;
        ByteArrayOutputStream outStream = new ByteArrayOutputStream();
        try {
            context = JAXBContext.newInstance(LoanHistorySheet.class);
            Marshaller marshaller = context.createMarshaller();
            marshaller.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, Boolean.TRUE);
            marshaller.marshal(loanHistorySheet, outStream);
    ...
    private ByteArrayOutputStream  generatePDF(StreamSource streamSource) {
        ...
        Optional<PdfDocumentDesign> optional = pdfDocumentDesignRepository
                .findByDocumentType(PdfDocumentDesign.LOAN_HISTORY);
