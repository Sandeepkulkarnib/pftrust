...
@Service
public class GenerateTransferOutDispatchLetter {

    private final PdfDocumentDesignRepository pdfDocumentDesignRepository;

    public GenerateTransferOutDispatchLetter(PdfDocumentDesignRepository pdfDocumentDesignRepository) {
        this.pdfDocumentDesignRepository = pdfDocumentDesignRepository;
    }

    public ByteArrayOutputStream generate(TransferOutDispatchLetter transferOutDispatchLetter) throws Exception {
        return generatePDFFromJavaObject(transferOutDispatchLetter);
    }

    private ByteArrayOutputStream generatePDFFromJavaObject(TransferOutDispatchLetter transferOutDispatchLetter) throws Exception {

        ByteArrayOutputStream xmlSource = getXMLSource(transferOutDispatchLetter);
    ...
    private ByteArrayOutputStream getXMLSource(TransferOutDispatchLetter transferOutDispatchLetter) throws Exception {
        JAXBContext context;
        ByteArrayOutputStream outStream = new ByteArrayOutputStream();
        try {
            context = JAXBContext.newInstance(TransferOutDispatchLetter.class);
            Marshaller marshaller = context.createMarshaller();
            marshaller.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, Boolean.TRUE);
            marshaller.marshal(transferOutDispatchLetter, outStream);
    ...
    private ByteArrayOutputStream  generatePDF(StreamSource streamSource)
        ...
        Optional<PdfDocumentDesign> optional = pdfDocumentDesignRepository
                .findByDocumentType(PdfDocumentDesign.TRANSFER_OUT_DISPATCH_LETTER);
