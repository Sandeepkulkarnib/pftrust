...
@Service
public class GenerateAnnuxurek {

    private final PdfDocumentDesignRepository pdfDocumentDesignRepository;

    public GenerateAnnuxurek(PdfDocumentDesignRepository pdfDocumentDesignRepository) {
        this.pdfDocumentDesignRepository = pdfDocumentDesignRepository;
    }

    public ByteArrayOutputStream generate(AnnuxureK annuxureK) throws Exception {
        return generatePDFFromJavaObject(annuxureK);
    }

    private ByteArrayOutputStream generatePDFFromJavaObject(AnnuxureK annuxureK) throws Exception {

        ByteArrayOutputStream xmlSource = getXMLSource(annuxureK);
    ...
    private ByteArrayOutputStream getXMLSource(AnnuxureK annuxureK) throws Exception {
        JAXBContext context;
        ByteArrayOutputStream outStream = new ByteArrayOutputStream();
        try {
            context = JAXBContext.newInstance(AnnuxureK.class);
            Marshaller marshaller = context.createMarshaller();
            marshaller.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, Boolean.TRUE);
            marshaller.marshal(annuxureK, outStream);
    ...
    private ByteArrayOutputStream  generatePDF(StreamSource streamSource) {
        ...
        Optional<PdfDocumentDesign> optional = pdfDocumentDesignRepository
                .findByDocumentType(PdfDocumentDesign.ANNUXURE_K);
