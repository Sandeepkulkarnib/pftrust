...
@Service
public class GenerateLoanWorksheet {

    private final PdfDocumentDesignRepository pdfDocumentDesignRepository;

    public GenerateLoanWorksheet(PdfDocumentDesignRepository pdfDocumentDesignRepository) {
        this.pdfDocumentDesignRepository = pdfDocumentDesignRepository;
    }

    public ByteArrayOutputStream generate(LoanWorkSheet loanWorkSheet) throws Exception {
        return generatePDFFromJavaObject(loanWorkSheet);
    }

    private ByteArrayOutputStream generatePDFFromJavaObject(LoanWorkSheet loanWorkSheet) throws Exception {

        ByteArrayOutputStream xmlSource = getXMLSource(loanWorkSheet);
    ...
    private ByteArrayOutputStream getXMLSource(LoanWorkSheet loanWorkSheet) throws Exception {
        JAXBContext context;
        ByteArrayOutputStream outStream = new ByteArrayOutputStream();
        try {
            context = JAXBContext.newInstance(LoanWorkSheet.class);
            Marshaller marshaller = context.createMarshaller();
            marshaller.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, Boolean.TRUE);
            marshaller.marshal(loanWorkSheet, outStream);
    ...
    private ByteArrayOutputStream  generatePDF(StreamSource streamSource)
        ...
        Optional<PdfDocumentDesign> optional = pdfDocumentDesignRepository
                .findByDocumentType(PdfDocumentDesign.LOAN_WORKSHEET);
