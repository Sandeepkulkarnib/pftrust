...
@Service
public class NewGenerateAnnualStatement {

    private final PdfDocumentDesignRepository pdfDocumentDesignRepository;

    public NewGenerateAnnualStatement(PdfDocumentDesignRepository pdfDocumentDesignRepository) {
        this.pdfDocumentDesignRepository = pdfDocumentDesignRepository;
    }

    public ByteArrayOutputStream generate(NewAnnualStatement annualStatement) throws Exception {
        return generatePDFFromJavaObject(annualStatement);
    }

    private ByteArrayOutputStream generatePDFFromJavaObject(NewAnnualStatement annualStatement) throws Exception {
    ...
    private ByteArrayOutputStream getXMLSource(NewAnnualStatement annualStatement) throws Exception {
        JAXBContext context;
        ByteArrayOutputStream outStream = new ByteArrayOutputStream();
        try {
            context = JAXBContext.newInstance(NewAnnualStatement.class);
            Marshaller marshaller = context.createMarshaller();
            marshaller.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, Boolean.TRUE);
            marshaller.marshal(annualStatement, outStream);

            OutputStream outputStream = new FileOutputStream("books.xml");
...
//        InputStream in = IOUtils.toInputStream(optional.get().getDocument(), StandardCharsets.UTF_8);
        ...
        InputStream in = Model.class.getClassLoader().getResourceAsStream("pdfs/new-annual-statement.xsl");
