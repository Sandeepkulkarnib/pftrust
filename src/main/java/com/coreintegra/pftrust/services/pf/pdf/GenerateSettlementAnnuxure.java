...
@Service
public class GenerateSettlementAnnuxure {

    private final PdfDocumentDesignRepository pdfDocumentDesignRepository;

    public GenerateSettlementAnnuxure(PdfDocumentDesignRepository pdfDocumentDesignRepository) {
        this.pdfDocumentDesignRepository = pdfDocumentDesignRepository;
    }

    public ByteArrayOutputStream generate(SettlementAnnexure settlementAnnexure) throws Exception {
        return generatePDFFromJavaObject(settlementAnnexure);
    }

    private ByteArrayOutputStream generatePDFFromJavaObject(SettlementAnnexure settlementAnnexure) throws Exception {

        ByteArrayOutputStream xmlSource = getXMLSource(settlementAnnexure);
    ...
    private ByteArrayOutputStream getXMLSource(SettlementAnnexure settlementAnnexure) throws Exception {
        JAXBContext context;
        ByteArrayOutputStream outStream = new ByteArrayOutputStream();
        try {
            context = JAXBContext.newInstance(SettlementAnnexure.class);
            Marshaller marshaller = context.createMarshaller();
            marshaller.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, Boolean.TRUE);
            marshaller.marshal(settlementAnnexure, outStream);
    ...
    private ByteArrayOutputStream  generatePDF(StreamSource streamSource)
        ...
        Optional<PdfDocumentDesign> optional = pdfDocumentDesignRepository
                .findByDocumentType(PdfDocumentDesign.SETTLEMENT_ANNEXURE);
